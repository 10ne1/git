			      ...
			      void *cb_data)
{
	...
	/* Look for "hook.friendlyname.event = hook_event" */
		  ...
		  struct run_hooks_opt *options)
{
	...
	/*
	 * feed_pipe_cb_data is an opaque pointer to the pp child callback data
	 * used to keep internal states for each hook, accesible via pp_task_cb.
	 * The initial value of this state sometimes needs to be duplicated, for
	 * e.g. when running hooks in parallel, therefore:
	 * If options->copy_feed_pipe_cb_data is set, the state gets duplicated
	 * Else it is reused.
	 */
